...
INTEGER :: ind, indx, indx1 ! loop index
...
REAL(KIND = R8) :: f
REAL(KIND = R8), DIMENSION(:), ALLOCATABLE :: f1_t, f2_t, x2_t
REAL(KIND = R8), DIMENSION(:,:), ALLOCATABLE :: x1_t
...
INTERFACE
  FUNCTION  Obj_TEST_B(c, iflag) RESULT(f)
		USE TestObjSur_MOD
		USE TestObj_MOD
...
! Loop calling VTdirect subroutine to optimize five test functions.
  ...
  DO ind = 1, 3, 1
	 ...
	 DO wt = 0, 1.1, 1
      ...
      IF (n_optbox == 1) THEN ! A single best box output is specified.
        CALL VTdirect(N, L, U, Obj_TEST_B, X, FMIN, status, SWITCH=c_switch,&
  ...
  153 FORMAT(" ----- End of test results for function",I2,". -----"//)
	...
	READ(75,*) numND;
  ALLOCATE(x1_t(numND,Nv))
  ALLOCATE(x2_t(Nv))
	DO indx=1, numND, 1
		READ(75,*) x2_t
		DO indx1 = 1, Nv, 1
			x1_t(indx1, indx) = x2_t(indx1)
		END DO
	END DO
	CLOSE(75)
	DO indx=1, numND, 1
		DO indx1 = 1, Nv, 1
			c(indx1) = x1_t(indx1,indx)
		END DO
		CALL TestObj(c,f)
	END DO
